...
const CustomListItem = ({id, chatName, enterChat}) => {
    ...
    useEffect(() => {
        const unsubscribe = db.collection('signal_chats').doc(id).collection('messages')
    ...
    return (
        <ListItem onPress={() => enterChat(id, chatName)} key={id} bottomDivide>
            <Avatar
                rounded
                source={{
                    uri: chatMessages?.[0]?.photoURL ||  'https://cencup.com/wp-content/uploads/2015/03/avatar-placeholder.png'
                }}
             />
